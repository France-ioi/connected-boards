...
export class SensorMotor extends AbstractSensor<any> {
  private pale: any;
  private center: any;
  private updatetimeout: any;
  public type = SensorType.Motor;

  static getDefinition(context: QuickalgoLibrary, strings: any): SensorDefinition {
    return {
      name: "motor",
      suggestedName: strings.messages.sensorNameMotor,
      description: strings.messages.motor,
      isAnalog: true,
      isSensor: false,
      portType: "D",
      valueType: "number",
  ...
  setLiveState(state, callback) {
    var command = "setMotorSpeed(\"" + this.name + "\"," + state + ")";

    this.context.quickPiConnection.sendCommand(command, callback);
  }

  draw(sensorHandler: SensorHandler, {imgx, imgy, imgw, imgh, juststate, fadeopacity, state1x, state1y}: SensorDrawParameters) {
    if (this.stateText)
